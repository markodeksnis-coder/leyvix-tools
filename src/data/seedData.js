// Deterministic seed data for Marko OS
// Called once on first open; checks marko_initialized flag
...
function _r(from, to) {
  // from = older (larger n), to = more recent (smaller n)
  const a = []
...
const HABITS = [
  ...
  {
    id: 'sales_content', name: 'Sales Content',
    logs: [..._r(80, 65), ..._r(55, 38), ..._r(25, 8), ..._r(5, 0)],
    fails: [_da(37), _da(7), _da(6)],
  },
  {
    id: 'nutrition', name: 'Nutrition On Point',
...
const MIND = [
  ...
  { id: 2, title: 'Status Signals in Conversation', category: 'Human Behavior', summary: 'High-status individuals speak less, interrupt more, and take up more physical space. They do not explain themselves. They make others qualify to them.', source: 'The Art of Power — Ury', date: _da(8), keyPrinciple: 'Less talking = more status' },
  ...
  { id: 6, title: 'The 2 Commandments of Relationships', category: 'Relationships', summary: 'Show up consistently and add more value than you take. Relationships die when the ratio flips. People keep who makes them feel good and elevated.', source: 'Personal observation', date: _da(22), keyPrinciple: 'Consistency + value = loyalty' },
]
...
const BODY = {
  ...
  workouts: [
    ...
    { id: 7, date: _da(8), name: 'Push Day', exercises: [{ name: 'Bench Press', sets: 4, reps: '9,8,7,6', weight: 185 }, { name: 'Incline DB Press', sets: 3, reps: '10,9,8', weight: 65 }] },
...
const RELATIONS = {
  people: [
    { id: 1, name: 'David Chen', type: 'Mentor', lastInteraction: _da(3), energyRating: 'builds', notes: 'Sales director. Met at conference. Runs a $5M door-to-door operation. Study his methods.', psychologyNotes: [{ id: 1, note: 'Only responds to direct asks — never hints', date: _da(10) }] },
    { id: 2, name: 'Jake Marini', type: 'Friend', lastInteraction: _da(1), energyRating: 'builds', notes: 'Closest friend. On same mission. Accountability partner for gym and business goals.', psychologyNotes: [] },
    { id: 3, name: 'Sarah Deksnis', type: 'Family', lastInteraction: _da(0), energyRating: 'neutral', notes: 'Sister. Good relationship. Needs consistent investment.', psychologyNotes: [] },
    { id: 4, name: 'God', type: 'God', lastInteraction: _da(0), energyRating: 'builds', notes: 'Prayer + scripture daily. He is the source of everything.', psychologyNotes: [] },
    { id: 5, name: 'Marcus Webb', type: 'Business', lastInteraction: _da(7), energyRating: 'neutral', notes: 'Potential partner. Still proving himself. Watch for consistency between words and actions.', psychologyNotes: [{ id: 1, note: 'Talks big but delays action — needs accountability', date: _da(7) }] },
    { id: 6, name: 'Uncle Ray', type: 'Family', lastInteraction: _da(14), energyRating: 'drains', notes: 'Negative energy. Loves to criticize ambition. Keep exposure low.', psychologyNotes: [{ id: 1, note: 'Projects insecurities onto others\' goals', date: _da(14) }] },
  ],
  behaviorNotes: [
    { id: 1, observation: 'Confident people fill silence', detail: 'Insecure people rush to fill silence with explanations. Confident people let silence breathe. Silence is power.', date: _da(4) },
    { id: 2, observation: 'People follow energy, not logic', detail: 'Nobody gets convinced by argument. They get moved by emotion and then justify it with logic after. Lead with feeling.', date: _da(9) },
    { id: 3, observation: 'How someone treats service workers', detail: 'Non-negotiable filter. Someone who disrespects waiters or cashiers reveals their true character the moment they have power.', date: _da(17) },
  ],
}
...
const BUSINESS = {
  ...
  lessons: [
    { id: 1, title: 'What killed the Martinez appointment', what: 'Knocked at 7pm, owner just got home from work. Tired, hungry, distracted.', learned: 'Never pitch tired people. Come back Saturday morning or weekday 6-7pm before dinner.', date: _da(3) },
    { id: 2, title: 'Thompson — what closed it', what: 'Led with neighborhood social proof. Three neighbors already had our system. Used names.', learned: 'Social proof from neighbors is the #1 door opener. Collect and use names aggressively.', date: _da(2) },
    { id: 3, title: 'Garcia — almost lost it at the close', what: 'Gave too many options. Confusion created hesitation. Wife started questioning.', learned: 'One path forward only. Eliminate choice at close. "The next step is X, when works better — Tuesday or Wednesday?"', date: _da(5) },
  ],
  revenueHistory: [
    ...[...Array(30)].map((_, i) => {
...
const SETTINGS = { name: 'Marko', apiKey: '' }
...
export function initSeedData() {
  ...
  localStorage.setItem('marko_settings', JSON.stringify(SETTINGS))
