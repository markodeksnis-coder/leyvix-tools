...
export default function Business() {
  ...
  const revenueColor = monthRevenue >= 10000 ? '#facc15' : monthRevenue >= 5000 ? 'white' : '#8b5cf6'
    ...
    <div className="h-full flex flex-col" style={{ background: '#0a0a0f' }}>
      ...
      <div className="px-6 py-4 shrink-0 flex items-center justify-between" style={{ borderBottom: '1px solid #1e1b2e' }}>
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowLessonModal(true)}
            style={{ background: 'transparent', color: '#6b7280', border: '1px solid #1e1b2e', borderRadius: 8, padding: '6px 12px', fontFamily: 'Inter', fontSize: 11, cursor: 'pointer' }}
      ...
      <div className="flex-1 overflow-auto px-6 py-5" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div style={{ ...CARD, borderColor: monthRevenue >= 10000 ? '#facc1540' : '#1e1b2e' }}>
            ...
            <div style={{ marginTop: 8, height: 4, background: '#1e1b2e', borderRadius: 2 }}>
              <div style={{ height: 4, background: monthRevenue >= 10000 ? '#facc15' : '#7c3aed', borderRadius: 2, width: `${Math.min(100, (monthRevenue / 10000) * 100)}%`, transition: 'width 0.3s' }} />
            </div>
          </div>
          {[
            ...
            {deals.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {deals.slice(0, 8).map(d => {
                  ...
                  return (
                    <div key={d.id} className="group flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[#0a0a0f] transition-colors">
                      ...
                      <select
                        ...
                        style={{ fontFamily: 'Inter', fontSize: 10, color: sc.color, border: `1px solid ${sc.border}`, background: '#0a0a0f', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', outline: 'none' }}
                      >
        ...
        <div style={CARD}>
          ...
          <div style={{ position: 'relative', marginBottom: 14 }}>
            ...
            <input
              ...
              style={{ width: '100%', background: '#0a0a0f', border: '1px solid #1e1b2e', borderRadius: 8, paddingLeft: 36, paddingRight: 12, paddingTop: 8, paddingBottom: 8, fontFamily: 'Inter', fontSize: 13, color: '#d1d5db', outline: 'none' }}
            />
          </div>
          {filteredLessons.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredLessons.map(l => (
                <div key={l.id} className="group" style={{ background: '#0a0a0f', border: '1px solid #1e1b2e', borderRadius: 8, padding: 14 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                    <div>
                      <span style={{ fontFamily: 'Inter', fontSize: 10, fontWeight: 700, color: '#8b5cf6', textTransform: 'uppercase', letterSpacing: '0.06em', marginRight: 8 }}>{l.category}</span>
                  ...
                  {l.learned && <p style={{ fontFamily: 'Inter', fontSize: 12, color: '#888', borderLeft: '2px solid #7c3aed40', paddingLeft: 10, marginTop: 6 }}>{l.learned}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <Modal title="Add Deal" onClose={() => setShowDealModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={addDeal} style={{ background: '#7c3aed', color: 'white', borderRadius: 8 }} className="flex-1 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity">Save</button>
              <button onClick={() => setShowDealModal(false)} style={{ border: '1px solid #1e1b2e', color: '#555', borderRadius: 8 }} className="px-4 py-2.5 text-[10px] uppercase tracking-widest hover:border-[#444] transition-colors">Cancel</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {showLessonModal && (
        <Modal title="Add Lesson" onClose={() => setShowLessonModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={addLesson} style={{ background: '#7c3aed', color: 'white', borderRadius: 8 }} className="flex-1 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity">Save</button>
              <button onClick={() => setShowLessonModal(false)} style={{ border: '1px solid #1e1b2e', color: '#555', borderRadius: 8 }} className="px-4 py-2.5 text-[10px] uppercase tracking-widest hover:border-[#444] transition-colors">Cancel</button>
            </div>
          </div>
        </Modal>
      )}
