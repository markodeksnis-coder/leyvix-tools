...
const ALERT_COLORS = {
  RECORD: { bg: '#14102a', border: '#8b5cf640', text: '#8b5cf6', icon: '#8b5cf6' },
  BODY: { bg: '#1a1005', border: '#f9731640', text: '#f97316', icon: '#f97316' },
  BUSINESS: { bg: '#1a1400', border: '#f59e0b40', text: '#f59e0b', icon: '#f59e0b' },
  RELATIONS: { bg: '#1a0a1a', border: '#ec489940', text: '#ec4899', icon: '#ec4899' },
  SYSTEM: { bg: '#111111', border: '#44444440', text: '#888', icon: '#555' },
}
...
const cls = {
  ...
  warInput: "w-full bg-[#0a0a0a] border border-[#1e1a2e] px-3 py-2 text-sm text-white placeholder-[#444] focus:outline-none focus:border-[#f97316] transition-colors rounded-lg",
...
function Sparkline({ logs, fails }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 2, alignItems: 'center', overflow: 'hidden' }}>
      {dots.map((status, i) => (
        <div key={i} style={{ width: 3, height: 10, borderRadius: 1.5, flexShrink: 0, background: status === 'done' ? '#22c55e' : status === 'failed' ? '#ef4444' : '#1a1a1a' }} />
      ))}
    </div>
  )
}
...
const SectionHeader = ({ children }) => (
  <div className="font-display text-white" style={{ fontSize: 22, borderLeft: '2px solid #f59e0b', paddingLeft: 12 }}>{children}</div>
)
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-display" style={{
              ...
              background: view === 'warroom' ? 'linear-gradient(135deg, #f97316 0%, #ec4899 100%)' : '#f59e0b',
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'Inter', fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, fontWeight: 600, color: view === 'warroom' ? '#f97316' : '#f59e0b' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: view === 'warroom' ? '#f97316' : '#f59e0b' }} />
          ...
          {[{ id: 'dashboard', label: 'Dashboard' }, { id: 'warroom', label: 'War Room Ledger' }].map(t => {
            ...
            return (
              <button key={t.id} onClick={() => setView(t.id)} style={{
                background: isActive ? (t.id === 'warroom' ? 'linear-gradient(135deg, #f97316, #ec4899)' : '#f59e0b') : 'transparent',
        ...
        {view === 'dashboard' ? (
          <>
            ...
            <section>
              ...
              {habits.length === 0 ? (
                <div style={{ background: '#0d0d0d', border: '1px solid #262626', borderRadius: 10, padding: '32px', textAlign: 'center', fontFamily: 'Inter', fontSize: 12, color: '#333' }}>No habits yet</div>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2.5">
                  {habits.map(h => {
                    ...
                    return (
                      <div key={h.id} style={{
                        background: '#0d0d0d', border: '1px solid #262626', borderLeft: `3px solid ${barColor}`,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontFamily: 'Inter', fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.icon} {h.name}</span>
                          <button onClick={() => { setHabitForm({ name: h.name, icon: h.icon || '🎯', category: h.category || 'Health', target: h.target || 'Daily' }); setEditHabit(h); setShowHabitModal(true) }} style={{ color: '#444', flexShrink: 0, marginLeft: 4 }} className="hover:text-[#888] transition-colors"><Pencil size={10} /></button>
                        </div>
                        ...
                        <div style={{ fontFamily: 'Inter', fontSize: 10, color: '#555' }}>BEST {streak.longest}d · WK {streak.weekCount} / MO {streak.monthCount}</div>
                  ...
                  {[trophies[1] ? { t: trophies[1], p: PODIUM[1] } : null, { t: trophies[0], p: PODIUM[0] }, trophies[2] ? { t: trophies[2], p: PODIUM[2] } : null].map((item, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: p.size, height: p.size, borderRadius: '50%', border: `8px solid ${p.stroke}`, boxShadow: `0 0 24px ${p.glow}`, background: '#0d0d0d', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <ResponsiveContainer width="100%" height={200}>
                      <BarChart data={chartData} margin={{ top: 16, right: 16, bottom: 52, left: 0 }}>
                        <XAxis dataKey="name" angle={-45} textAnchor="end" interval={0} tick={{ fill: '#666', fontSize: 11, fontFamily: 'Inter' }} axisLine={{ stroke: '#262626' }} tickLine={false} />
                        <YAxis tick={{ fill: '#666', fontSize: 11, fontFamily: 'Inter' }} axisLine={{ stroke: '#262626' }} tickLine={false} />
                        <Tooltip contentStyle={{ background: '#0d0d0d', border: '1px solid #262626', borderRadius: 8, fontSize: 12, fontFamily: 'Inter' }} labelStyle={{ color: '#888' }} itemStyle={{ color: '#fff' }} cursor={{ fill: 'rgba(255,255,255,0.02)' }} />
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowContentModal(true)} style={{ border: '1px solid #262626', color: '#555', fontFamily: 'Inter' }} className="flex items-center gap-1 px-3 py-1 text-[9px] uppercase tracking-widest hover:text-white hover:border-[#f59e0b] transition-colors">
              ...
              <div className="flex gap-1.5 flex-wrap mb-4">
                {CONTENT_CATS.map(cat => {
                  ...
                  return (
                    <button key={cat} onClick={() => setContentCat(cat)} style={{ background: isActive ? color : '#1a1a1a', color: isActive ? (cat === 'All' ? '#0a0a0a' : 'white') : '#555', fontFamily: 'Inter', fontSize: 11, fontWeight: isActive ? 600 : 400, padding: '4px 12px', borderRadius: 9999, border: 'none', cursor: 'pointer', transition: 'all 0.15s' }}>{cat}</button>
                  )
                })}
              </div>
              {filteredContent.length === 0 ? (
                <div style={{ background: '#0d0d0d', border: '1px solid #262626', borderRadius: 10, padding: '32px', textAlign: 'center', fontFamily: 'Inter', fontSize: 12, color: '#333' }}>No entries yet</div>
              ) : (
                <div style={{ columnCount: 2, columnGap: 12 }}>
                  {filteredContent.map(c => {
                    const color = CAT_COLORS[c.category] || '#6b7280'
                    return (
                      <div key={c.id} className="group" style={{ background: '#0d0d0d', border: '1px solid #262626', borderRadius: 10, borderLeft: `4px solid ${color}`, padding: '12px 14px', marginBottom: 10, breakInside: 'avoid', transition: 'box-shadow 0.15s' }}
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <button onClick={() => deleteContent(c.id)} style={{ color: '#333' }} className="hover:text-[#ef4444] opacity-0 group-hover:opacity-100 transition-all"><X size={11} /></button>
                          </div>
                        </div>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, alignItems: 'start' }}>
              ...
              {/* Today's Orders */}
              <div style={{ ...WAR_CARD, borderColor: '#f9731630', boxShadow: '0 0 28px rgba(249,115,22,0.06)' }}>
                <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 14 }}>
                  <div className="flex items-center gap-2">
                    ...
                    <span style={{ background: '#f9731618', border: '1px solid #f9731640', borderRadius: 4, padding: '2px 8px', fontFamily: 'Inter', fontSize: 9, fontWeight: 700, color: '#f97316', letterSpacing: '0.06em' }}>OPERATOR AI</span>
                  </div>
                  <button
                    onClick={generateOrders}
                    disabled={ordersLoading}
                    style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'transparent', border: '1px solid #f9731640', color: '#f97316', borderRadius: 6, padding: '5px 10px', fontFamily: 'Inter', fontSize: 10, fontWeight: 600, cursor: ordersLoading ? 'wait' : 'pointer', letterSpacing: '0.04em' }}
                  >
                ...
                {ordersError ? (
                  <div style={{ fontFamily: 'Inter', fontSize: 12, color: '#ef4444', padding: '12px 0' }}>
                    {ordersError.includes('No API key') ? (
                      <span>No API key set. <button onClick={() => { const k = prompt('Paste your Anthropic API key:'); if (k) { localStorage.setItem('anthropic_key', k.trim()); setOrdersError('') } }} style={{ color: '#f97316', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontFamily: 'Inter', fontSize: 12, padding: 0 }}>Set key</button></span>
                    ) : ordersError}
                  </div>
                ) : orders ? (
                  <div style={{ fontFamily: 'Inter', fontSize: 13, color: '#d1d5db', lineHeight: 1.7 }}>
                    {orders.split('\n').map((line, i) => {
                      ...
                      if (bold) return <div key={i} style={{ marginTop: i > 0 ? 8 : 0 }}><span style={{ color: '#f97316', fontWeight: 700 }}>{bold[1]}</span><span>{bold[2]}</span></div>
                  ...
                  <div style={{ fontFamily: 'Inter', fontSize: 12, color: '#444', padding: '16px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Sparkles size={14} color="#f97316" />
                    <span>Click REGENERATE to get your daily orders from Operator AI</span>
                  </div>
                )}

                <div style={{ marginTop: 16, height: 3, background: '#1a1a1a', borderRadius: 2 }}>
                  <div style={{ width: `${readinessPct}%`, height: 3, background: 'linear-gradient(90deg, #f97316, #ec4899)', borderRadius: 2, transition: 'width 0.5s' }} />
                </div>
                <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#f97316', marginTop: 6, letterSpacing: '0.06em', fontWeight: 600 }}>READINESS: {readinessPct}%</div>
              </div>

              {/* Log Morning + Evening */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div
                  style={{ ...WAR_CARD, padding: 16, cursor: morningLogged ? 'default' : 'pointer', borderColor: morningLogged ? '#22c55e40' : '#1e1a2e' }}
                  onClick={() => !morningLogged && setShowMorningModal(true)}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Sun size={13} color={morningLogged ? '#22c55e' : '#f97316'} />
                    ...
                    {morningLogged ? (
                      ...
                      <div style={{ width: 22, height: 22, borderRadius: '50%', border: '1px solid #f9731660', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f97316' }}>
                        <Plus size={12} />
                      </div>
                    )}
                  </div>
                  <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', marginTop: 6, letterSpacing: '0.06em' }}>SLEEP · ENERGY · MOOD</div>
                  {morningLogged && (
                    <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                      {[['💤', todayCheckLog.morning?.sleep, 'h'], ['⚡', todayCheckLog.morning?.energy, '/10'], ['😊', todayCheckLog.morning?.mood, '/10']].map(([icon, val, unit]) => val ? (
                        <span key={icon} style={{ fontFamily: 'Inter', fontSize: 10, color: '#888' }}>{icon} {val}{unit}</span>
                      ) : null)}
                    </div>
                  )}
                </div>
                ...
                <div
                  style={{ ...WAR_CARD, padding: 16, cursor: eveningLogged ? 'default' : 'pointer', borderColor: eveningLogged ? '#22c55e40' : '#1e1a2e' }}
                  onClick={() => !eveningLogged && setShowEveningModal(true)}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Moon size={13} color={eveningLogged ? '#22c55e' : '#8b5cf6'} />
                    ...
                    {eveningLogged ? (
                      ...
                      <div style={{ width: 22, height: 22, borderRadius: '50%', border: '1px solid #8b5cf640', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8b5cf6' }}>
                        <Plus size={12} />
                      </div>
                    )}
                  </div>
                  <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', marginTop: 6, letterSpacing: '0.06em' }}>STRESS · WORK · TRAINING</div>
                  {eveningLogged && (
                  ...
                  return (
                    <div key={i} style={{ background: ac.bg, border: `1px solid ${ac.border}`, borderRadius: 8, padding: '12px 14px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontFamily: 'Inter', fontSize: 11, color: '#888', lineHeight: 1.5 }}>{alert.body}</div>
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div style={WAR_CARD}>
                ...
                <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginTop: 2 }}>{dayName}, {dateLabel}</div>
                <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    { label: 'MISSION DONE', value: `${doneCount}/${habits.length}`, pct: missionPct, color: '#f97316' },
                    { label: 'ACTIVE STREAKS', value: `${activeStreaks}/${habits.length}`, pct: activePct, color: '#f97316' },
                    { label: 'FAILED TODAY', value: `${failedCount}/${habits.length}`, pct: failedPct, color: '#ef4444' },
                  ].map(row => (
                    <div key={row.label}>
                      <div className="flex items-center justify-between" style={{ fontFamily: 'Inter', fontSize: 10, color: '#888', letterSpacing: '0.05em', marginBottom: 5 }}>
                        <span>{row.label}</span>
                ...
                <div style={{ display: 'flex', gap: 20, marginTop: 20, paddingTop: 14, borderTop: '1px solid #1a1a1a' }}>
                  {[{ label: 'BEST EVER', value: bestEver }, { label: 'PERFECT DAYS', value: perfectDays }, { label: 'HABITS', value: habits.length }].map(s => (
                    <div key={s.label}>
                      <div className="font-display" style={{ fontSize: 22, color: '#f97316', lineHeight: 1 }}>{s.value}</div>
                      <div style={{ fontFamily: 'Inter', fontSize: 8, color: '#555', marginTop: 3, letterSpacing: '0.05em' }}>{s.label}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={WAR_CARD}>
                ...
                <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginTop: 2 }}>COMPOSITE RECOVERY / 7-DAY</div>
                <div style={{ marginTop: 16, display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <span className="font-display" style={{ fontSize: 52, lineHeight: 1, background: fatigueIndex.score >= 70 ? 'linear-gradient(135deg, #f97316, #ec4899)' : fatigueIndex.score >= 40 ? '#f59e0b' : '#ef4444', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{fatigueIndex.score}</span>
                  <span style={{ fontFamily: 'Inter', fontSize: 14, fontWeight: 700, color: fatigueIndex.score >= 70 ? '#f97316' : '#888' }}>{fatigueIndex.label}</span>
                </div>
                <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', marginBottom: 10 }}>OUT OF 100</div>
                <ResponsiveContainer width="100%" height={48}>
                  <AreaChart data={fatigueChart} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                    ...
                    <Area type="monotone" dataKey="score" stroke="#f97316" strokeWidth={1.5} fill="url(#fatigueGrad)" dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
                {fatigueIndex.drivers.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginBottom: 8 }}>DRIVERS</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {fatigueIndex.drivers.map((d, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <span style={{ fontFamily: 'Inter', fontSize: 10, color: '#888' }}>{d.label}</span>
                ...
                {!morningLogged && (
                  <button onClick={() => setShowMorningModal(true)} style={{ marginTop: 12, width: '100%', background: 'transparent', border: '1px solid #f9731630', borderRadius: 6, padding: '6px', fontFamily: 'Inter', fontSize: 10, color: '#f97316', cursor: 'pointer', letterSpacing: '0.04em' }}>
              ...
              <div style={WAR_CARD}>
                ...
                <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginTop: 2, marginBottom: 14 }}>PERSONAL BESTS</div>
                {topHabits.length === 0 ? (
                  <div style={{ fontFamily: 'Inter', fontSize: 12, color: '#333', textAlign: 'center', padding: '32px 0' }}>No records yet</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {topHabits.slice(0, 6).map((h, i) => (
                      <div key={h.id} className="flex items-center justify-between gap-2">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden' }}>
                          <span style={{ fontFamily: 'Inter', fontSize: 9, color: i === 0 ? '#f59e0b' : '#555', flexShrink: 0, fontWeight: 700 }}>#{String(i + 1).padStart(2, '0')}</span>
                          <span style={{ flexShrink: 0 }}>{h.icon}</span>
                          <span style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 600, color: i === 0 ? 'white' : '#888', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{h.name.toUpperCase()}</span>
                        </div>
                        <span style={{ fontFamily: 'Inter', fontSize: 12, color: i === 0 ? '#f59e0b' : '#666', fontWeight: 700, flexShrink: 0 }}>
                          {h.best}<span style={{ color: '#444', fontWeight: 400, fontSize: 9 }}>d</span>
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            <div style={WAR_CARD}>
              <div className="flex items-center justify-between" style={{ marginBottom: 4 }}>
                ...
                <div style={{ display: 'flex', gap: 10, fontFamily: 'Inter', fontSize: 9, color: '#888' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 7, height: 7, background: '#f97316', borderRadius: 2 }} /> CURRENT</span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 7, height: 7, background: '#7a2e00', borderRadius: 2 }} /> BEST</span>
                </div>
              </div>
              <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginBottom: 8 }}>CURRENT VS BEST</div>
              {habits.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={streakChartData} margin={{ top: 8, right: 0, bottom: 0, left: 0 }} barGap={2} barCategoryGap="30%">
                    <XAxis dataKey="name" tick={{ fill: '#555', fontSize: 8, fontFamily: 'Inter' }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#444', fontSize: 9, fontFamily: 'Inter' }} axisLine={false} tickLine={false} width={22} />
                    <Tooltip contentStyle={{ background: '#0d0d0d', border: '1px solid #1e1a2e', borderRadius: 8, fontFamily: 'Inter', fontSize: 11 }} labelStyle={{ color: '#f97316' }} itemStyle={{ color: '#fff' }} cursor={{ fill: 'rgba(249,115,22,0.04)' }} />
                    <Bar dataKey="Current" fill="#f97316" radius={[2, 2, 0, 0]} />
            ...
            <div style={WAR_CARD}>
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div style={{ fontFamily: 'Inter', fontSize: 9, color: '#555', letterSpacing: '0.06em', marginTop: 3 }}>LIVE STATUS · {habits.length} HABITS ENGRAVED</div>
                </div>
                <div className="flex items-center gap-2">
                  <div style={{ position: 'relative' }}>
                    ...
                    <input value={habitSearch} onChange={e => setHabitSearch(e.target.value)} placeholder="Search habits..."
                      style={{ background: '#0a0a0a', border: '1px solid #1e1a2e', borderRadius: 6, padding: '7px 10px 7px 30px', fontFamily: 'Inter', fontSize: 11, color: 'white', width: 160, outline: 'none' }} />
                  </div>
                  <button onClick={() => { setHabitForm({ name: '', icon: '🎯', category: 'Health', target: 'Daily' }); setEditHabit(null); setShowHabitModal(true) }}
                    style={{ background: 'linear-gradient(135deg, #f97316, #ec4899)', color: 'white', border: 'none', borderRadius: 6, padding: '7px 12px', fontFamily: 'Inter', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
              ...
              {habits.length === 0 ? (
                ...
                <>
                      ...
                      <thead>
                        <tr style={{ borderBottom: '1px solid #1e1a2e' }}>
                          {['HABIT', 'CURRENT', 'EFFICIENCY', 'BEST', 'STATUS', 'COMMAND'].map(h => (
                            <th key={h} style={{ fontFamily: 'Inter', fontSize: 9, fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: 'left', padding: '0 10px 10px', whiteSpace: 'nowrap' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                            ...
                            <tr key={h.id} style={{ borderBottom: '1px solid #111' }}>
                              <td style={{ padding: '11px 10px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                                  <span style={{ fontFamily: 'Inter', fontSize: 9, color: '#444' }}>{String(i + 1).padStart(2, '0')}</span>
                                  <span>{h.icon}</span>
                                  <span style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 700, color: 'white' }}>{h.name.toUpperCase()}</span>
                                  <button onClick={() => { setHabitForm({ name: h.name, icon: h.icon || '🎯', category: h.category || 'Health', target: h.target || 'Daily' }); setEditHabit(h); setShowHabitModal(true) }} style={{ color: '#333', cursor: 'pointer', background: 'none', border: 'none', padding: 0 }}><Pencil size={9} /></button>
                                </div>
                              </td>
                              <td style={{ padding: '11px 10px' }}>
                                <span className="font-display" style={{ fontSize: 20, color: '#f97316' }}>{String(streak.current).padStart(2, '0')}</span>
                                <span style={{ fontFamily: 'Inter', fontSize: 9, color: '#555' }}>d</span>
                              </td>
                              <td style={{ padding: '11px 10px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                                  <div style={{ width: 70, height: 3, background: '#1a1a1a', borderRadius: 2, flexShrink: 0 }}>
                                    <div style={{ width: `${effPct}%`, height: 3, background: 'linear-gradient(90deg, #f97316, #ec4899)', borderRadius: 2 }} />
                                  </div>
                                  <span style={{ fontFamily: 'Inter', fontSize: 10, color: '#888' }}>{effPct}%</span>
                                </div>
                              </td>
                              <td style={{ padding: '11px 10px', fontFamily: 'Inter', fontSize: 12, color: '#666' }}>{streak.longest}d</td>
                              <td style={{ padding: '11px 10px' }}>
                                ...
                                {status === 'RECORD' && <span style={{ fontFamily: 'Inter', fontSize: 9, fontWeight: 700, color: '#0a0a0a', background: '#f59e0b', borderRadius: 4, padding: '3px 7px' }}>🏆 RECORD</span>}
                              ...
                              <td style={{ padding: '11px 10px' }}>
                                {doneToday || failedToday ? (
                                  <button onClick={() => undoHabit(h.id)} style={{ fontFamily: 'Inter', fontSize: 10, color: '#666', border: '1px solid #333', borderRadius: 5, padding: '4px 9px', background: 'transparent', cursor: 'pointer' }}>UNDO</button>
                                ) : (
                                  <div style={{ display: 'flex', gap: 5 }}>
                                    <button onClick={() => logHabit(h.id)} style={{ fontFamily: 'Inter', fontSize: 10, fontWeight: 700, color: 'white', background: 'linear-gradient(135deg, #f97316, #ec4899)', border: 'none', borderRadius: 5, padding: '4px 9px', cursor: 'pointer' }}>DONE</button>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #111' }}>
                    <div style={{ display: 'flex', gap: 14, fontFamily: 'Inter', fontSize: 9, color: '#555' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 7, height: 7, borderRadius: 2, background: '#f59e0b' }} />RECORD</span>
                    ...
                    <span style={{ fontFamily: 'Inter', fontSize: 9, color: '#555' }}>{filteredHabits.length} / {habits.length} SHOWN</span>
                  </div>
                </>
              )}
            </div>
      ...
      {showMorningModal && (
        <Modal title="Log Morning Check-In" onClose={() => setShowMorningModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={saveMorningLog} style={{ background: 'linear-gradient(135deg, #f97316, #ec4899)', color: 'white', borderRadius: 8 }} className="flex-1 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity">Log Morning</button>
              <button onClick={() => setShowMorningModal(false)} style={{ border: '1px solid #262626', color: '#555', borderRadius: 8 }} className="px-4 py-2.5 text-[10px] uppercase tracking-widest hover:border-[#444] transition-colors">Cancel</button>
            </div>
          </div>
        </Modal>
      )}
        ...
        <Modal title="Log Evening Check-In" onClose={() => setShowEveningModal(false)}>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              <input type="checkbox" id="training" checked={eveningForm.training} onChange={e => setEveningForm({ ...eveningForm, training: e.target.checked })} style={{ accentColor: '#f97316', width: 16, height: 16 }} />
              <label htmlFor="training" style={{ fontFamily: 'Inter', fontSize: 12, color: '#d1d5db', cursor: 'pointer' }}>Trained today</label>
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={saveEveningLog} style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', borderRadius: 8 }} className="flex-1 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity">Log Evening</button>
              <button onClick={() => setShowEveningModal(false)} style={{ border: '1px solid #262626', color: '#555', borderRadius: 8 }} className="px-4 py-2.5 text-[10px] uppercase tracking-widest hover:border-[#444] transition-colors">Cancel</button>
            </div>
          </div>
        </Modal>
      )}
